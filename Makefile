...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"pddf_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f pddf-*.tar.gz
	echo Create archive \"pddf-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../pddf-$(VERSION)-$(BRANCH).tar.gz pddf
...
pddf.tar.gz: build
	cd make_build; tar czf ../pddf.tar.gz pddf && rm -rf ../make_build
